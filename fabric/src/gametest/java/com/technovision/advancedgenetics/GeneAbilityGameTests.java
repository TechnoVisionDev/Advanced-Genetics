...
public class GeneAbilityGameTests {
    ...
    public void grassMilkHoneyMeatAndWoolKeepOriginalInteractions(GameTestHelper helper) {
        ...
        UseBlockCallback.EVENT.invoker().interact(actor, helper.getLevel(), InteractionHand.MAIN_HAND, hit);
    ...
    public void witherHitAndInfinityRetainCombatBehavior(GameTestHelper helper) {
        ...
        helper.assertTrue(AttackEntityCallback.EVENT.invoker().interact(player, helper.getLevel(), InteractionHand.MAIN_HAND, target, null)
    ...
    public void deathGenesAndKeepInventoryCopyRemainAvailable(GameTestHelper helper) {
        ...
        helper.assertTrue(ServerPlayerEvents.ALLOW_DEATH.invoker().allowDeath(original, original.damageSources().generic(), 50),
                "Death genes allow death to proceed");
        helper.assertItemEntityPresent(Items.EMERALD);
        helper.assertEntityPresent(EntityTypes.SLIME);
        ...
        helper.assertTrue(ComponentRegistry.PLAYER_GENETICS.get(restored).hasGene(Genes.KEEP_INVENTORY), "CCA copies genes to the respawned player");
    ...
    public void wallClimbingWebWalkingAndFearGoalsRemainInstalled(GameTestHelper helper) throws ReflectiveOperationException {
        ...
        for (Mob mob : new Mob[]{helper.spawn(EntityTypes.CREEPER, POS), helper.spawn(EntityTypes.SKELETON, POS)}) {
    ...
    private static void interact(GameTestHelper helper, ServerPlayer actor, ServerPlayer donor) {
        UseEntityCallback.EVENT.invoker().interact(actor, helper.getLevel(), InteractionHand.MAIN_HAND, donor, null);
    }
}
